...
# Exercise 1:
evens_to_20 = [x*2 for x in upto_ten]

# Exercise 2:
upto_ten_odd = [x for x in upto_ten if x%2 != 0]
    
# Exercise 3:
upto_10_squares = [x*x for x in upto_ten]

# Exercise 4:
upto_10_and_square = [[x, x*x] for x in upto_ten]
# Exercise 5:
...
flattened = [x for blah in matrix for x in blah]

# Exercise 8:
names = ['Jim', 'Jan', 'Zaphod', 'David', 'Bart', 'Lisa']
initials = [x[0] for x in names]
...
ids = [x[0] for x in employees]

# Exercise 11:
high_pay_rates = [[x[1],x[0]] for x in employees if x[3] >= 15]

# Exercise 12:
colors = ['red','green','yellow']
things = ['tree','house','ball']
colored_things = ["{}".format(x +" "+ y) for x in colors for y in things]
